...
EIGHTEEN_DECIMAL_NUMBER: constant(uint256) = 10 ** 18
...
def getRawPiOutput(error: int256, errorI: int256) -> (int256, int256, int256):
    ...
    pOutput: int256 = (error * self.kp) // convert(EIGHTEEN_DECIMAL_NUMBER, int256)
    iOutput: int256 = (errorI * self.ki) // convert(EIGHTEEN_DECIMAL_NUMBER, int256)

    return (self.coBias + pOutput + iOutput, pOutput, iOutput)

@external
@view
def getRawOutput(error: int256, errorI: int256) -> (int256, int256, int256):
    # // output = P + I = Kp * error + Ki * errorI
    pOutput: int256 = (error * self.kp) // convert(EIGHTEEN_DECIMAL_NUMBER, int256)
    iOutput: int256 = (errorI * self.ki) // convert(EIGHTEEN_DECIMAL_NUMBER, int256)
